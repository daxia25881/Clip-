...
public class PasteboardItemRepresentation: NSManagedObject
{
    ...
    /// Creates a plain text representation for background saving (e.g., from WebDAV upload).
    public static func makePlainText(_ text: String, in context: NSManagedObjectContext) -> PasteboardItemRepresentation
    {
        let representation = PasteboardItemRepresentation(uti: "public.utf8-plain-text", type: .text, context: context)
