...
private extension UserDefaults
{
    ...
    @NSManaged var lastPasteboardChangeCount: Int
}
...
private class PersistentContainer: RSTPersistentContainer
{
    ...
    {
        guard let appGroup = Bundle.main.appGroups.first,
              let sharedDirectoryURL = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: appGroup)
        else {
            print("DatabaseManager: App Group not found or invalid, falling back to local documents.")
...
public extension DatabaseManager
{
    func savePasteboard(location: CLLocation?, completionHandler: @escaping (Result<Void, Error>) -> Void)
    {
        do
        {
            ...
            guard UIPasteboard.general.changeCount != UserDefaults.shared.lastPasteboardChangeCount else {
            ...
            PasteboardItemRepresentation.representations(for: itemProvider, in: context) { (representations) in
                do
                {
                    ...
                    DispatchQueue.main.async {
                        UserDefaults.shared.lastPasteboardChangeCount = UIPasteboard.general.changeCount
    ...
    /// Saves a plain text string directly to Core Data.
    /// Used for background saving when text is uploaded via WebDAV.
    public func save(text: String, completionHandler: @escaping (Result<Void, Error>) -> Void)
    {
        let context = DatabaseManager.shared.persistentContainer.newBackgroundContext()
        ...
        context.perform {
            do
            {
                // Create representation with public.utf8-plain-text UTI
                let representation = PasteboardItemRepresentation.makePlainText(text, in: context)
                
                // Create the PasteboardItem
                guard let pasteboardItem = PasteboardItem(representations: [representation], context: context) else {
                    throw PasteboardError.noItem
                }
                
                // Check for duplicate (compare with most recent item)
                let fetchRequest = PasteboardItem.fetchRequest() as NSFetchRequest<PasteboardItem>
                fetchRequest.predicate = NSPredicate(format: "%K == NO", #keyPath(PasteboardItem.isMarkedForDeletion))
                fetchRequest.sortDescriptors = [NSSortDescriptor(keyPath: \PasteboardItem.date, ascending: false)]
                fetchRequest.relationshipKeyPathsForPrefetching = ["representations"]
                ...
                if let previousItem = try context.fetch(fetchRequest).first,
                   let previousText = previousItem.preferredRepresentation?.stringValue,
                   previousText == text
                {
                    throw PasteboardError.duplicateItem
                }
                ...
                DispatchQueue.main.async {
                    print("DatabaseManager: Successfully saved text to Core Data.")
                    completionHandler(.success(()))
                }
            }
            catch
            {
                DispatchQueue.main.async {
                    print("DatabaseManager: Failed to save text.", error)
                    completionHandler(.failure(error))
                }
            }
        }
