...
class NotificationViewController: UIViewController, UNNotificationContentExtension
{
    ...
    func didReceive(_ notification: UNNotification)
    {
        ...
        // Handle "CopyToPasteboard" action from PendingMessageManager
        if let action = notification.request.content.userInfo["Action"] as? String,
           action == "CopyToPasteboard",
           let text = notification.request.content.userInfo["Text"] as? String
        {
            UIPasteboard.general.string = text
            print("NotificationViewController: Copied text to clipboard.")
            self.extensionContext?.dismissNotificationContentExtension()
            return
        }
